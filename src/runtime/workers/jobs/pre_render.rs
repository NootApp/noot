...
pub async fn pre_render(_job: Job, worker: &mut Worker, window: Id, path: PathBuf) -> Option<JobList> {
    if path.exists() {
        ...
        let images = find_image(buffer.id.clone(), &buffer.doc, worker, window, path);
...
fn find_image(bid: String, els: &[ElWrapper], worker: &mut Worker, window: Id, path: PathBuf) -> JobList {
    ...
    for el in els {
        if el.name == "img" {
            image_jobs.push(
                Job::new(
                    JobType::CacheAsset(path.clone(), Url::from_str(el.attributes.get("src").unwrap().clone().unwrap().as_str()).unwrap(), window, bid.clone())
                )
            )
        } else if el.children.len() > 0 {
            image_jobs.append(&mut find_image(bid.clone(), &el.children, worker, window, path.clone()));
        }
    }
