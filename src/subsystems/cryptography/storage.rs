...
pub fn store(
    ...
    if enterprise {
        ...
        let mut cipher =
            SymmetricCipher::new_from_id("symmetric_workspace").unwrap();
    ...
    if let Ok(mut handle) = handle_result {
        ...
        return match magic {
            &CONSUMER_MAGIC => {
                ...
                let mut cipher =
                    SymmetricCipher::new_from_id("symmetric_workspace")
                        .unwrap();

                let decrypt_outcome =
                    cipher.decrypt_with_nonce(&nonce, &buffer[13..]);
                ...
                if decrypt_outcome.is_ok() {
                    ...
                    error!(
                        "Decryption failed: {:?}",
                        decrypt_outcome.clone().unwrap_err()
                    );
...
mod tests {
    ...
    pub fn test_encryption() {
        ...
        store(
            &PathBuf::from_str("./sample.txt").unwrap(),
            raw.as_bytes(),
            false,
        )
        .unwrap();
